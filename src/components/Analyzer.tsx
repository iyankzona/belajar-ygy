...
function MetricTile({
  ...
  return (
    <div
      className="relative flex flex-col gap-1 p-4 bg-surface border border-border rounded-xl"
    ...
    >
      <div className="flex items-center gap-1">
        ...
        {tooltip && (
          <svg className="w-3 h-3 text-muted shrink-0" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
      ...
      {show && tooltip && (
        <div className="absolute bottom-full left-0 mb-2 z-50 w-56 rounded-lg bg-text text-surface text-xs leading-relaxed p-3 shadow-lg pointer-events-none">
          {tooltip}
          <div className="absolute top-full left-4 w-0 h-0 border-l-4 border-r-4 border-t-4 border-l-transparent border-r-transparent border-t-text" />
        </div>
      )}
    </div>
  )
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-3">
                <MetricTile
                  ...
                  tooltip="The total amount spent across all campaigns in the selected period."
                />
                <MetricTile
                  label="Total Revenue"
                ...
                <MetricTile
                  ...
                  sub={view === 'weekly' ? 'per day (7-day week)' : `per day (${selectedPeriod ? getDaysInPeriod(selectedPeriod, view) : 30}-day month)`}
                  tooltip={`Total spend divided by the number of days in the selected ${view === 'weekly' ? 'week (7 days)' : 'month'}. Use this to set your daily budget in Google Ads.`}
                />
                <MetricTile
                  label="Incr. Spend"
                  value={formatCurrency(metrics.totalIncrementalSpend)}
                  sub="vs prior period"
                  tooltip="The additional spend compared to the previous period. Only counts campaigns where spend increased. This is the marginal investment being evaluated."
                />
                <MetricTile
                  label="Incr. Revenue"
                  value={formatCurrency(metrics.totalIncrementalRevenue)}
                  sub="vs prior period"
                  tooltip="The additional revenue generated by the incremental spend. Positive means the extra spend is paying off; negative means it is not."
                />
                <MetricTile
                  label="Overall iROAS"
                  ...
                  tooltip={`Incremental ROAS: Incremental Revenue ÷ Incremental Spend. This measures the efficiency of the additional spend only. Your target is ${targetIncrementalRoas}. Green = above target, Red = below target.`}
                />
              </div>
