...
function VarianceBadge({ pct, positiveIsGood = true }: { pct: number; positiveIsGood?: boolean }) {
  const isPositive = pct >= 0
  const goodColor = isPositive === positiveIsGood ? 'text-scale' : 'text-reduce'
  const arrow = isPositive ? '▲' : '▼'
  const absLabel = `${Math.abs(pct).toFixed(1)}%`
  return (
    <span className={`inline-flex items-center gap-0.5 text-[11px] font-semibold tabular-nums ${goodColor}`}>
      {arrow} {absLabel}
    </span>
  )
}
...
function MetricTile({
  ...
  tooltip?: string
  alignRight?: boolean
  /** Percentage change vs prior period (positive = increase). Null = no prior data. */
  ...
  return (
    ...
    >
      ...
      {variance != null && <VarianceBadge pct={variance} positiveIsGood={positiveIsGood} />}
      {show && tooltip && (
...
export function Analyzer() {
  ...
  // Prior-period consolidated metrics for variance calculations
  ...
  const priorMetrics = useMemo(() => {
    ...
    const rows = consolidateRows(
      sourceRows, view, targetIncrementalRoas, minimumSpend, reallocationPercentage,
      priorPeriod, lastActionDates, CADENCE_DAYS[decisionCadence], weeklyRows,
    )
    return computeConsolidatedMetrics(rows)
  }, [priorPeriod, sourceRows, view, targetIncrementalRoas, minimumSpend, reallocationPercentage, lastActionDates, decisionCadence, weeklyRows])

  /** Returns % change of `current` vs `prior`, or null if either is 0/null. */
  function pctChange(current: number, prior: number | null | undefined): number | null {
    if (!prior || prior === 0 || current === 0) return null
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 divide-y sm:divide-y-0 divide-x-0 sm:divide-x divide-border">
                  <div className="px-5 py-4">
                    <MetricTile
                      ...
                      variance={pctChange(metrics.totalSpend, priorMetrics?.totalSpend)}
                  ...
                  <div className="px-5 py-4">
                    <MetricTile
                      ...
                      variance={pctChange(metrics.totalRevenue, priorMetrics?.totalRevenue)}
                    />
                  </div>
                  <div className="px-5 py-4">
                    <MetricTile
                      ...
                      variance={pctChange(metrics.averageRoas ?? 0, priorMetrics?.averageRoas)}
                    />
                  </div>
                  <div className="px-5 py-4">
                    <MetricTile
                  ...
                  tooltip={`Total spend ÷ days in the selected ${view === 'weekly' ? 'week (7 days)' : view === 'biweekly' ? 'biweekly period (14 days)' : 'month'}. Use this as your daily budget cap in Google Ads.`}
                      alignRight
                      variance={pctChange(avgDailySpend ?? 0, priorPeriod && priorMetrics ? priorMetrics.totalSpend / getDaysInPeriod(priorPeriod, view) : null)}
                      positiveIsGood={false}
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 divide-y sm:divide-y-0 divide-x-0 sm:divide-x divide-border">
                  <div className="px-5 py-4">
                    <MetricTile
                      ...
                      variance={pctChange(metrics.totalIncrementalSpend, priorMetrics?.totalIncrementalSpend)}
                      positiveIsGood={false}
                    />
                  </div>
                  <div className="px-5 py-4">
                    <MetricTile
                      ...
                      variance={pctChange(metrics.totalIncrementalRevenue, priorMetrics?.totalIncrementalRevenue)}
                    />
                  </div>
                  <div className="px-5 py-4 col-span-2 sm:col-span-1">
                    <MetricTile
                      ...
                      alignRight
                      variance={pctChange(metrics.overallIncrementalRoas ?? 0, priorMetrics?.overallIncrementalRoas)}
                    />
                  </div>
                </div>
